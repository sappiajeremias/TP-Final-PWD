...
;(function ($, window, document, undefined) {
    ...
    if (sessionStorage.getItem('carro')){
        var carritoCache = sessionStorage.getItem('carro');
        $.each(JSON.parse(carritoCache), function(key, value) {
            value.product_id = parseInt(value.product_id);
            carrito[key] = value;
        });
    }

    var defaults = {
        cart: carrito, // initial products on cart
        ...
        lang: { // Language variables
            ...
            cartRemove: '<i class="fa-solid fa-xmark"></i>',
            cartEmpty: 'Carrito vacio!<br />Busque productos'
        },
    ...
    $.extend(SmartCart.prototype, {
        ...
        _setElements: function () {
            ...
            this.cart_element.append('<div class="panel-heading sc-cart-heading">' + this.options.lang.cartTitle + ' <span class="sc-cart-count badge bg-warning">0</span></div>');
        ...
        _setEvents: function () {
            ...
            $(this.cart_element).on("change", '.sc-cart-item-qty', function (e) {
                ...
                sessionStorage.setItem('carro', JSON.stringify(this.cart));
            });
        ...
        /* 
         * ELIMINA UN OBJETO PRODUCTO DEL CARRITO
         */
        _removeFromCart: function (unique_key) {
            ...
            $.each(this.cart, function (i, n) {
                if (n.unique_key === unique_key) {
                    ...
                    mi._hasCartChange();
                    sessionStorage.setItem('carro', JSON.stringify(mi.cart));
                    console.log(sessionStorage.getItem('carro'));
        ...
        _updateCartQuantity: function (unique_key, qty) {
            ...
            $.each(this.cart, function (i, n) {
                if (n.unique_key === unique_key) {
                    ...
                    carrito = sessionStorage.getItem('carro');
                    carrito[i].product_quantity = qty;
                    sessionStorage.setItem('carro', carrito);
                    console.log(sessionStorage.getItem('carro'))
        ...
        _addUpdateCartItem: function (p) {
            ...
            if (elmMain && elmMain.length > 0) {
                ...
                itemSummary += ' X <input type="number" min="1" max="1000" class="sc-cart-item-qty" value="' + this._getValueOrEmpty(p[this.options.paramSettings.productQuantity]) + '" />';
